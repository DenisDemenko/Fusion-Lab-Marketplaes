...
- [x] Кейс-стаді "як я використовував Claude Code для оркестрації" —
      [`docs/case-study-ai-orchestration.md`](docs/case-study-ai-orchestration.md)
      (англійською — цільова аудиторія документа, на відміну від ADR/
      ROADMAP, це рекрутер/наймаючий менеджер, не я сам). Побудований на
      реальних фактах з git-історії й цієї роботи: цикл "будуй → перевір
      незалежно → зафіксуй рішення", конкретний приклад бага
      (`localePrefix`), знайденого саме незалежною перевіркою, а не
      довірою до звіту сабагента, і межа делегування (сабагентам —
      механічна робота за вже підтвердженим патерном, не судження).
